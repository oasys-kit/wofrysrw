...
from syned.beamline.shape import HyperbolicCylinder

try: from wofrysrw.util.srw import SRWLOptMirHyp
except ImportError: SRW_OLD = True
else:               SRW_OLD = False

class SRWHyperbolicMirror(SRWMirror):
    def __init__(self,
                 name                                       = "Undefined",
        ...
        if SRW_OLD: raise ValueError("installed SRW version does not contain Hyperbolic mirrors")

        shape = HyperbolicCylinder()
        shape.initialize_from_p_q(distance_from_first_focus_to_mirror_center,
    ...
    def get_SRWLOptMir(self, nvx, nvy, nvz, tvx, tvy, x, y, ap_shape):
        ...
        return SRWLOptMirHyp(_size_tang=self.tangential_size,
    ...
    def fromSRWLOpt(self, srwlopt):
        if not isinstance(srwlopt, SRWLOptMirHyp):
            raise ValueError("SRW object is not a SRWLOptMirHyp object")
    ...
    def to_python_code_aux(self, nvx, nvy, nvz, tvx, tvy, x, y, ap_shape):
        ...
        text_code  = "SRWLOptMirHyp(_size_tang=" + str(self.tangential_size) +"," + "\n"
