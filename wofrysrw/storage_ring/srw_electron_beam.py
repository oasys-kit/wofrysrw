...
class SRWElectronBeam(ElectronBeam, SRWElectronBeamDecorator, SRWObject):
    def __init__(self,
        ...
        self.set_first_moments_all(moment_x, moment_xp, moment_y, moment_yp, moment_z)

    def set_first_moments_all(self, moment_x=0.0, moment_xp=0.0, moment_y=0.0, moment_yp=0.0, moment_z=0.0):
    ...
    def get_first_moments_all(self):
        return self._moment_x, \
               self._moment_xp, \
               self._moment_y, \
               self._moment_yp, \
               self._moment_z
    ...
    def to_SRWLPartBeam(self):
        ...
        srw_electron_beam.arStatMom2[1]  = moment_xxp  # <(x-x0)*(x'-x'0)> [m]
        srw_electron_beam.arStatMom2[2]  = moment_xpxp # <(x'-x'0)^2>
        srw_electron_beam.arStatMom2[3]  = moment_yy   #<(y-y0)^2>
        srw_electron_beam.arStatMom2[4]  = moment_yyp  #<(y-y0)*(y'-y'0)> [m]
        srw_electron_beam.arStatMom2[5]  = moment_ypyp #<(y'-y'0)^2>
    ...
    def to_python_code(self, data=None):
        ...
        text_code += "part_beam.arStatMom2[2]      = " + str(moment_xpxp) + "\n"
        text_code += "part_beam.arStatMom2[3]      = " + str(moment_yy) + "\n"
        text_code += "part_beam.arStatMom2[4]      = " + str(moment_yyp) + "\n"
        text_code += "part_beam.arStatMom2[5]      = " + str(moment_ypyp) + "\n"
